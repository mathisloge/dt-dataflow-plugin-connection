...
void Udp::connect()
{
    if (options_.listen_protocol != UdpOptions::Protocol::none)
    {
        ...
        listen_endpoint_ = udp::endpoint(options_.listen_protocol == UdpOptions::Protocol::ipv4 ? udp::v4() : udp::v6(),
...
void Udp::applyOptions()
{
    ...
    if (!options_.write_address.empty() && options_.write_port > 0)
        write_endpoint_ = boost::asio::ip::udp::endpoint(boost::asio::ip::address::from_string(options_.write_address),
                                                         options_.write_port);
}
                      ...
                      std::size_t /*bytes_transferred*/)
{
    if (error)
    {
        SPDLOG_ERROR("Error while writing to target: {} err: {}, bytes should have been written: {}",
                     write_endpoint_.address().to_string(),
                     error.message(),
                     buffer_tx->size());
    }
}
