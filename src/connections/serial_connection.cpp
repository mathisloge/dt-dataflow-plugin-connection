...
void Serial::handleRead(const boost::system::error_code &e, std::size_t bytes_transferred)
{
    if (!e)
    {
        ...
        serial_.async_read_some(
            ...
            std::bind(&Serial::handleRead, shared_from_this(), std::placeholders::_1, std::placeholders::_2));
    }
    else
    {
    ...
    if (!error)
    {
        if (buffer_tx->size() != bytes_transferred)
        {
            ...
            serial_.async_write_some(
                ...
                std::bind(
                    &Serial::handleWrite, shared_from_this(), buffer_tx, std::placeholders::_1, std::placeholders::_2));
        }
    }
    else
    {
...
void Serial::write(std::span<uint8_t> data)
{
    ...
    serial_.async_write_some(
        boost::asio::buffer(*buffer_tx),
        std::bind(&Serial::handleWrite, shared_from_this(), buffer_tx, std::placeholders::_1, std::placeholders::_2));
}
